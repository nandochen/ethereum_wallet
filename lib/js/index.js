...
//============================================================================= 
// 錢包地址 / Wallet Address 
var myAddress = '0x24B7ABB3f771CCf88024Da62120795ef3bd1B87A';
// 私鑰，正式環境請勿暴露此資訊!! / Private key, DO NOT SHOW IN LIVE ENV.!!                                                         
var myPK = '0x05c1ba034282dcfb3350807b1dc41fe4edd073583b00138e17b7633b7bbdcbb9';
// Etherscan API 密鑰，正式環境請勿暴露此資訊!! / EtherScan API KEY, DO NOT SHOW IN LIVE ENV.!!                                                                
var ESAPIKey = 'E5CSKCC5SWWP3DAZDXK26ZNKMGSDE3HZX6';                                                                  
...
function confirmTransaction() {
    ...
    if (valid) {
        ...
        if (confirmTrans) {
            ...
            // 進行交易，調用發送通正功能
            // Commit transaction, invoke send token function 
            sendToken(address, amount, function(result){
                // 交易已發送，隱藏鎖定資訊
                // Transaction sent, hide lock info 
                ...
                // 交易失敗，隱藏鎖定資訊、顯示取消按鈕與錯誤訊息
                // Transaction failed, hide lock info, show cancel button and error info 
                $('#viewTransferLock').modal('hide');  
 ...
 * 發送通證 
 * Send token 
 * @param {*} _address - 地址 / Address 
 * @param {*} _amount - 數量 / Amount 
 * @param {*} _callback - 成功回調功能 / Callback function on success  
 * @param {*} _error - 失敗回調功能 / Callback function on error   
 */
function sendToken(_address, _amount, _callback, _error) {
    ...
    contractSigner.transfer(
        _address /* 接收方地址 / Address */
        , ethers.utils.parseUnits(_amount.toString(), USDT.decimal).toString()          /* 數量，依序通證小數點轉換 / Amount */
        , { 
            gasPrice: ethers.utils.parseUnits((defaultGASPrice).toString(), 'gwei'),    /* GAS 價格 / GAS Price */
            gasLimit: defaultGASLimit + (Math.floor(Math.random() * 100))               /* GAS 上限 / GAS limit  */
        }
    ).then(function(tx) {  
        // 執行回覆功能 
        // Callback 
